...
CHART_IMAGES_DIR = Path(os.environ.get("FACEBOOK_CHART_IMAGES_DIR", "/var/lib/trading-bot/facebook/chart-images"))
    ...
    try:
        ...
        if chart_file and chart_file.filename:
            try:
                ...
                image_path = CHART_IMAGES_DIR / f"{data['signal_id']}.png"
                chart_file.save(str(image_path))
                data["chart_image"] = str(image_path)
                log(f"Chart image saved for {data['signal_id']} -> {image_path}")
            except Exception as e:
                log(f"Chart image save failed for {data['signal_id']}: {e}")
...
if __name__ == "__main__":
    ...
    log("=" * 52)
    log("Signal Server started → https://testmachine.store")
    log("MT5 whitelist URL   → https://testmachine.store")
    log("EA InpFacebookURL   → https://testmachine.store/signal")
