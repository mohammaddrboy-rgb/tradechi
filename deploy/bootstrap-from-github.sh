...
if command -v nginx >/dev/null; then
  cp -f "${BOT_ROOT}/deploy/nginx/testmachine.store.conf" /etc/nginx/sites-available/testmachine.store
  ln -sf /etc/nginx/sites-available/testmachine.store /etc/nginx/sites-enabled/testmachine.store
...
echo "Bootstrap complete — https://testmachine.store/"
