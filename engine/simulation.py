...
class SimulationTracker:
    ...
    def _dedupe_key(self, signal: dict, signal_time: datetime) -> str:
        # The engine registers a signal twice: once live (microsecond
        # timestamp) via runner.py, and again on every restart via
        # import_signal_log() replaying signal_log.txt, whose timestamps
        # are second-precision only (the Facebook bridge writes them with
        # isoformat(timespec="seconds")). trade_id() hashes the raw
        # timestamp string, so those two precisions produced different
        # ids and INSERT OR IGNORE never caught the duplicate. Truncating
        # to whole seconds here — for id purposes only — makes both paths
        # agree on the same id for the same real-world signal.
        return self.trade_id({**signal, "timestamp": signal_time.replace(microsecond=0).isoformat()})
